...
- **PSD pipeline service (new)** — wraps the existing Python package: `psd-html emit` + Grammar-G conformance, plus a web port of the GUI's "Edit links…" form. Emit, conformance, intake validation and link-slot discovery are already prototyped as working API endpoints ([api/main.py](../api/main.py): `/convert`, `/conformance`, `/validate`, `/link-candidates`), tested end-to-end against a real campaign PSD on Render. **The per-region fidelity gate is deliberately NOT in this service yet** — it needs headless Chromium via Playwright (~200–300 MB image bloat + real per-request CPU/RAM, a bad fit for a small container already doing PSD compositing), and where/whether it runs in the cloud path is an open infra decision (OQ-11). Until then, the emit-time deterministic checks (Word-metric line-count certification, overflow flags, safety refusals) plus human side-by-side review stand in for it. Timing, measured: HTML-only convert ≈ 18–20 s — within normal HTTP timeouts, so a synchronous API suffices for the HTML leg today; an async job model becomes necessary once the fidelity gate (Chromium render + pixel diff) is added and/or jobs chain into the OFT worker (the oft-quoted 60–120 s is the *full* PSD→OFT pipeline, not this leg).
...
| OQ-11 | Fidelity gate in the cloud path: the per-region pixel gate needs headless Chromium/Playwright (~200–300 MB + real CPU/RAM per run) and is currently absent from the cloud HTML service — run it on a beefier paid container, a separate async worker, keep it desktop/Windows-only, or accept human side-by-side review as the interim gate? Goal 4 in §1 ("distribution does not degrade QA guarantees") is not met for PSD jobs until this is decided. What's unique to the gate (not covered by the emit-time deterministic checks): invented/missing background bands, wrong image crops, and whole sections silently dropped by the solver | Decides whether cloud PSD builds get automated visual QA or ship on human review; sizes the container/tier budget | Adrian / eng + Kai |
